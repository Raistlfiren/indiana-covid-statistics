...
class RaceChart extends React.Component {
    ...
    componentDidMount() {
        this.getRaceDetails(this.props.countyName);
    }

    getRaceDetails(countyName) {
        axios.get('http://localhost/api/county/' + countyName + '/race')
            .then(result => this.setState({
                raceDetails: result.data,
                isLoading: false
            }))
    ...
    render() {
        ...
        let series = {
            series: [{
                name: 'Cases',
                data: this.state.raceDetails.map(elem => elem.covidCount)
            }, {
                name: 'Deaths',
                data: this.state.raceDetails.map(elem => elem.covidDeaths)
            }]
        }
        ...
        let options = {
            ...
            xaxis: {
                categories: this.state.raceDetails.map(elem => elem.race)
            },
        }
                    ...
                    <div className="card overflow-hidden">
                        <div className="card-body">
                            <div className="d-flex justify-content-between align-items-baseline mb-4 mb-md-3">
                                <h6 className="card-title mb-0">Race</h6>
